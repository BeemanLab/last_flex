...
function main() {
    ...
    console.log(my_url);
    var requestID;
    //This is where it starts communicating with server.js
    params = ServerHelper.empirical_start(url);
    ServerHelper.start_request();
    console.log("start received?", ServerHelper.start_received, "start requested?", ServerHelper.start_requested);
    console.log("params", params.group);
    ...
    function cfgIsReady() {
        ...
        if (ServerHelper.config_file.length > 0 ) {
            //as long as there is a config file with something in it, parse it and put it in the var cfg
            ...
            //if the cfg wasn't loaded (i.e. the system hasn't checked if it's empty yet), just chill until it's there
            window.requestAnimationFrame(cfgIsReady);
    ...
    function preload_images(cfg) {
        //loop through the image filenames in cfg and use it to tell the system what the paths are for each image.
        console.log('called preload_images');
        for (var i=0; i < cfg.images.length; i++){
            var im = new Image();
            im.src = ServerHelper.image_url + 'study_1_img/' + cfg.images[i];
            //console.log('im', im);
            imagesOfImages.push(im);
            console.log(ServerHelper.image_url);
        }
        preload_done = 1;
    ...
    function ready_to_start(){
        if (preload_done === 1){
            //if we're ready to start, start, the function start_CRA_experiment is loccated in CRA_experiment.js.
