function start_CRA_experiment(cfg) {
	...
	function message(lines) {
       ...
       for (var i=0; i<lines.length; i++) {
		   ...
		   ctx.fillText(lines, WIDTH/ 2, HEIGHT / 2);
       }
   }
	...
	function initiateExperiment() {
		...
		var fsm = StateMachine.create({
			...
			events: [
				//{name: 'start', from: 'none', to: ['demo', 'instructions']},
                //{name: 'demo', from: 'start', to: 'instructions'},
				//{name: 'instructions', from: ['start', 'demo'], to: 'ready'},
				...
				{name: 'problem', from: 'iti', to: ['solution', 'moveToNext', 'grayscale']},
				{name: 'solution', from: ['practice', 'problem'], to: ['IorA', 'moveToNext']},
				//{name: 'IorA', from: 'solution', to: 'moveToNext'},
				{name: 'IorA', from: 'solution', to: 'grayscale'},
				{name: 'grayscale', from: ['IorA', 'problem'], to: 'imageConfirm'},
				{name: 'imageConfirm', from: 'grayscale', to: 'moveToNext'},
				{
					name: 'timeoutFlag',
				...
				{
					name: 'moveToNext',
					from: ['practice', 'problem', 'solution', 'IorA', 'imageConfirm'],
			...
			callbacks: {
				...
				onpractice: function (event, from, to) {
					var timeout = setTimeout(function () {
							...
							response.IorA = "NA";
							NAcount_prob++;
							//console.log('problems', cfg.images.length);
							fsm.onmoveToNext();
						},
					...
					if (practice) {
						...
						show_picture(trialNum);
						trialNum++;
						// trialNum is zero at beginning of practice.
				...
				onIorA: function (event, from, to) {
					//console.log("onIorA");
					ctx.clearRect(0, 0, WIDTH, HEIGHT);
					message(["1. Instant recognition, 2. Insight, or 3. Analysis?"]);
					//ctx.fillText("1. Instant recognition, \n 2. Insight, or \n 3. Analysis?", WIDTH / 2, HEIGHT / 2);
					var timeout = setTimeout(function () {
				...
				ongrayscale: function (event, from, to) {
					ctx.clearRect(0, 0, WIDTH, HEIGHT);

					var timeout = setTimeout(function () {
							console.log("Grayscale Timeout");
					...
					console.log("grayscale state");
					console.log(trialNum+80);
					show_picture(trialNum+80);
					window.onkeydown = function (e) {
						if (e.keyCode === 32) {
							//fsm.onmoveToNext();
							fsm.onimageConfirm();
				...
				onimageConfirm: function (event, from, to) {
					/*solBoxExist = document.getElementById("textbox");

					if (!!solBoxExist) {
						document.body.removeChild(solutionPrompt);
					}*/
					console.log("in imageconfirm");
					ctx.clearRect(0, 0, WIDTH, HEIGHT);
					message(["Was that the image you thought you saw?", "Press 1 if yes, press 2 if no"]);
					window.onkeydown = function (e) {
						if (e.keyCode === 49 || e.keyCode === 50) {
							ctx.clearRect(0, 0, WIDTH, HEIGHT);
							fsm.onmoveToNext();
							imageConfirmInput = String.fromCharCode(e.keyCode);
							response.imageConfirm = imageConfirmInput;
						}
                    }
				...
				onmoveToNext: function (event, from, to) {
					...
					else if (trialNum == (Math.floor(cfg.images.length / 2))) {
						blockNum++;
						fsm.onbreak();
					}
					else if (trialNum < cfg.images.length) {
						fsm.onready();
					}
					else {
